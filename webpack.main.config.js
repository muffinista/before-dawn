...
const outputDir = path.join.join(__dirname, "output");
...
let mainConfig = {
  ...
  entry: {
    main: path.join.join(__dirname, "src", "main", "index.js")
  },
  externals: deps,
  module: {
    rules: [
  ...
  plugins: [
    ...
    new CopyWebpackPlugin({
      patterns: [
        {
          from: path.join.join(__dirname, "package.json"),
          to: path.join.join(outputDir)
        },
        {
          from: path.join.join(__dirname, "src", "main", "assets"),
          to: path.join.join(outputDir, "assets"),
        },
        {
          from: path.join.join(__dirname, "src", "main", "system-savers"),
          to: path.join.join(outputDir, "system-savers"),
        }
      ]
    })
  ],
...
if (process.env.NODE_ENV === "production") {
  ...
  mainConfig.plugins.push(
    new webpack.DefinePlugin({
      "__static": `"${path.join.join(__dirname, "..", "static").replace(/\\/g, "\\\\")}"`
    })
  );
}
